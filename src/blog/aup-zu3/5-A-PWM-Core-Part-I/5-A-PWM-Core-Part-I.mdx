...
image:
    ...
    width: 300
tags: ["AUP-ZU3", "Tutorial", "Helpful Hint", "Simple Projects"]
---
...
This 90-minute lab will show how to create a simple RTL core
...
on how humans perceive high-frequencies).
...
The lab stops with a waveform but leaves the project set for the
...
While many IDEs have defaults that make it easy to start a project,
we prefer to create a directory structure ("folders") to organize
...
has the settings64.sh sourced and which does not.  You can
...
<details>
    ...
    If you are a seasoned hardware designer and fluent in HDL, skip ahead
    ...
    default peripheral bus clock speed is 100 MHz. This is really fast for human
    ...
    outputting low. During the second part we count a bunch of ($N$ number)
    of high-frequency clock cycles while outputting high. Then repeat.
    ...
    2. The first observation is that throughout our description we kept
    referring to ``counting clock'' cycles. This gives us a huge hint.
    ...
    teach FSM by starting with a block design that uses a register,
    a next-state function (what happens on the rising clock edge of the
    input clock), and an output function. And a counter is a
    ...
    once the maximum is reached or (2) sets the input to be one more than
    ...
    * a next-state function that will change the register every clock
    ...
    and customarily uses a reset signal to say, "start". So in the
    pseudocode we have chosen to check first for whether there is a
    ...
    --- you should check for your target technology. An experienced
    ...
    we are setting the next value to 0 in two places, we
    might want to refactor this code before formalizing it. The
    ...
    It turns out that it makes no difference to the compiler/synthesis tool
    ...
    if-statement less complex than doubly-nested one, even though the
    ...
    so we might settle on `period` and `low` and rely on the
    ...
    the documentation to answer it.  So we call this good enough
    ...
    we have simulated the core, it will be very convenient to
    see the progress and will tell us to look at changes to toggle.
    Furthermore, if we do not connect the output to anything in
    synthesis, the tools will trim away all of the logic needed
    to produce the output.  So it costs us nothing but might be
    helpful in debugging, so we map it out to the generic name `Q`.
    ...
    To this we need to add the ieee library and import the symbols
...
<details>
    ...
    our core and generate a skeleton testbench.  The first one that
    ...
    I find working with a standalone editor faster and more familiar
    ...
    When I plug in my entity, I got a start that retooled for what I wanted
    ...
    revisions and reuse --- you want to a have tool to automate unit testing
